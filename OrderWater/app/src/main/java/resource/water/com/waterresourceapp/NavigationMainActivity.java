...
public class NavigationMainActivity extends CommonActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkPermissions() {
        ...
        if (!permissions.isEmpty()) {
            ActivityCompat.requestPermissions(this, permissions.toArray(new String[permissions.size()]), REQUEST_CODE_PERMISSIONS);
        }
    }
            ...
            case REQUEST_CODE_PERMISSIONS: {
                for (int i = 0; i < permissions.length; i++) {
                    if (grantResults[i] == PackageManager.PERMISSION_DENIED) {
                        Toast.makeText(this, getString(R.string.enable_permissions), Toast.LENGTH_LONG).show();
                        finish();
                    }
                }
            }
